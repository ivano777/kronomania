...
### Refactors
- **Discipline effect-handler refactor (COMPLETE — 5 phases)** — dissolves the scattered discipline special-cases in the async spine into a uniform effect-handler layer. Verified debt: `_resolve_attack` hard-coded 3 disciplines by name (hex amp, Time Lock armed→frozen, `did_breach`), casts dispatched by `spell.spell_name` string match, ~12-key untyped `stat_overrides` payload, 0/6 pairwise interaction tests, Mind Detonation untested. **Phase 0**: `tests/integration/test_discipline_matrix.gd` — 10 characterization tests (Mind Detonation standalone + all 6 discipline pairs), frozen contract (stayed green unedited through Phases 1–4). **Phase 1**: typed views over the `stat_overrides` dict — `combat/effects/CastSnapshot.gd` + `MindBombPayload`/`EchoPayload`/`TimeLockPayload` + `AttackContext`; production reads/writes payloads only through these (schema in one place), storage unchanged so the net stayed frozen. **Phase 2**: `combat/effects/CombatEffect.gd` (base, 5 optional hooks: `on_wound_calc`/`on_player_attack_resolved`/`on_end_of_round`/`on_guard_reset`/`on_expire`) + `EffectRegistry` (status_id → handler) + `HexEffect`/`TimeLockEffect`/`EchoEffect`/`MindBombEffect`. The inline hex/Time-Lock/echo/frozen blocks moved into handlers; `_resolve_attack`/`_end_of_round`/`_process_statuses_hook` fire generic registry loops and name no discipline; `StatusOps.tick` 4-branch if-chain → one `on_expire`. **Phase 3**: `SpellData.cast_handler`/`placement_scratch`/`primes_status` fields replace the `spell.spell_name` cast-dispatch match (set on `mind_rend`/`time_lock`/`mind_detonation` .tres); echo arming stays tag-driven. **Phase 4**: `_run_enemy_attacks(slow_phase, p_total, target_pool, target_index, brutal_trade)` extracted; the three round coroutines' duplicated Phase 1/3 enemy loops collapse to one call each (cantrip/spell enemy-attack log now shows the enemy weapon name — a deliberate consistency unify with the strike round). CombatManager 1466 → 1394 lines; `combat/effects/` = 11 files (~313 lines); tests 125 → 135. Plan: `~/.claude/plans/i-think-we-have-twinkly-steele.md`.
- **CombatManager decomposition (in progress — incremental strangler)** — reducing the 2052-line god-object to an orchestrator + signal facade. Phase 0: 18 characterization contract-tests pin the async spine (`test_resolve_attack.gd`, `test_discipline_interactions.gd`) — must stay green unedited. Phase 1: `CombatantState` extracted to `combat/CombatantState.gd` (global `class_name`). Phase 2: pure math (stat/tier/keep/modifier/format) extracted to `combat/CombatMath.gd` (stateless `static`); CombatManager keeps same-signature delegating wrappers, so call sites + tests are unchanged; `test_combat_math.gd` covers the module standalone. Phase 3: status/interrupt bookkeeping extracted to `combat/StatusOps.gd` + `combat/InterruptOps.gd` (stateless `static`); `StatusOps.tick()` returns log lines the wrapper emits; the outcome-effect predicates/dispatch moved out of CombatManager entirely (5 wrappers deleted, `_apply_spell_outcome_effects` orchestrator retained); `test_status_ops.gd` covers both modules standalone. Awaited resolvers + `_process_statuses_hook` stay in CombatManager. Phase 4: the four magic disciplines (Mind Detonation, Mind Rend, Time Lock, Echoing Mind) extracted to `combat/Disciplines.gd` as statics over the CombatManager autoload global; the duplicated enemy-guard-roll block (`_cast_mind_rend`/`_cast_time_lock`) unified into `Disciplines.roll_enemy_guard`; CM keeps delegating wrappers (call sites + contract tests unchanged). Two chrono snippets stay inline in the spine (armed→frozen in `_resolve_attack`, frozen restore in `_end_of_round`). **COMPLETE — all 5 phases.** CombatManager 2052 → 1466 lines (−29%), 82 → 76 funcs; `combat/` = 5 modules, 773 lines; tests 85 → 125. Plan: `~/.claude/plans/analyze-the-project-and-linear-feather.md`.
...
- **Player battle sprite (beginner hero)** — first pipeline-authored combatant art. Idle = 4-frame sheet (breath bob + blink, 3 fps loop) via per-anim sheet support in `SpriteRegistry.get_combatant_frames` (`<anim>_sheet.{png,json}` preferred over `<anim>.png`; shared `_add_sheet_animation` helper with `get_effect_frames`); attack_melee/cast_spell/hurt/die = single-frame poses. *(Superseded by the 640x360 conversion below — figure redrawn at 64x72.)*
- **640x360 pixel-grid conversion (homogeneous art style)** — viewport 640x360 integer-scaled 2x into a 1280x720 window (`stretch=viewport`, `scale_mode=integer`); standard characters ≈64px tall. All four combatants redrawn as native-grid XPM pixel art (player 64x72: idle 4-step sheet + 4 poses; grunt 56x48 goblin, soldier 64x72, knight 64x80: idle 2-frame sheets + hurt + die — attack/cast slots empty by design, engine never plays them on enemies; legacy 500x500 art and 64x96 placeholder rects deleted). Install convention = scale 1 (old `--scale 15` dead). **VT323 pixel font** (OFL, `assets/fonts/`) wired as theme `default_font` @16 with AA/hinting/subpixel off; stylebox margins/radii shrunk. Full UI halving pass: BattleScene world anchors + HUD offsets, CombatantHUD (96 min-width, 10px wound slots), RoundHUD (anchor-driven width, 120px log), Campfire/MainMenu margins, tooltip sizes (140px, fonts→16), debug widget corners, Constellation canvas (all 18 node positions + vertices + card size 70x32 + font ladder 16/32 + line width 1). Tests 145 green; constellation layout may want one hand-tune pass after visual inspection.
- **CC0 combatant art (LuizMelo packs)** — pipeline-drawn character sprites replaced by converted CC0 asset packs (Martial Hero → player, Goblin → grunt, Medieval Warrior 2 → soldier, Hero Knight → knight; licenses in `assets/sprites/combatants/LICENSE.md`). New `tools/sprites/import_pack.py` + `packs.json`: strip slicing, union-bbox crop (feet alignment), facing flip, per-anim fps/loop → `<anim>_sheet.{png,json}` (loader unchanged). Full anim sets incl. enemy attacks; `die` freezes on last frame. `Combatant.gd` idle-return now via `animation_finished` (timers removed); new `CombatManager.combatant_attacking` signal animates enemy attacks in BattleScene. XPM pipeline scope narrowed to icons/effects/particles/VFX/UI. Tests 144 green. *(Supersedes the pipeline craft pass below.)*
- **Sprite craft pass (Blasphemous-grade)** — all 19 combatant frames redrawn to reference-quality pixel art: unified hue-shifted palette ramps (teal-steel/wine/parchment/gold/blood-red, outline `#181420` on silhouette only), cluster shading, asymmetric poses, weapons as solid overlay parts (player: low diagonal sword + red sash; knight: point-down greatsword in both gauntlets + tall pointed great helm; soldier: kettle helm + ochre tabard; grunt: hunched goblin + knife). Idle animation = blink + travelling blade glint (body-bob dropped — seams on asymmetric art). Craft rules + character palette codified in the pixel-sprites skill. Frame counts/fps unchanged; 145 tests + headless green.
- **UI readability + environment/UI art pass** — fixed the "glitchy" text (VT323 rendered off its pixel grid at 16px → swapped to **monogram**, CC0, on-grid at 16; ~14 uncovered symbol glyphs ASCII-ified across scenes + debug widgets to stop AA system-font fallback). Fixed CampfireScene bottom-cut (rest-button effect text → tooltips, weapon buttons → VBox, footer → single row, tighter spacing) + battle PlayerHUD band clip (80→120px). New `tests/integration/test_ui_bounds.gd` asserts Campfire/MainMenu fit 640x360 (caught a real `TextureRect` min-size overflow). **Scene backgrounds**: CC0 ansimuz Gothicvania packs (Cemetery → battle+campfire, Town → menu) composited by new `tools/sprites/import_env.py` + `env.json` → `assets/sprites/backgrounds/<key>.png`, loaded null-safe via new `SpriteRegistry.get_background`. **UI skin**: CC0 Kenney Fantasy UI Borders baked to two-tone 9-slice frames by new `tools/sprites/bake_ui.py` → `assets/sprites/ui/`, wired as `StyleBoxTexture` in `theme/dark_fantasy.tres` (panels + buttons framed). 146 tests + headless green. Licenses: `assets/sprites/{backgrounds,ui}/LICENSE.md`, `assets/fonts/LICENSE-monogram.txt`.
- **Constellation graphical overhaul + node detail panel** — text-only skill cards replaced by **icon tokens** (`SpriteRegistry.get_icon("nodes", node_id)` at 32px, 2-letter initials fallback for the 5 discipline nodes with no art), each with a graphical level-pip strip and a **per-branch coloured border** (`BRANCH_COLORS`: dominion crimson · negation steel-blue · ingenuity arcane-purple · core gold · flavor parchment, resolved by new `_branch_of()` since the `category` field is inconsistent). Prerequisite lines now tint to the child's branch colour when met. Node details moved from the cramped right-click float to a new **dismissable overlay `NodeDetailPanel`** (`class_name`, right-anchored ~190px) that shows the FULL per-level ladder — cost, tier gate, prerequisites, effect, granted spells, `uses_per_combat`, and `bonus_effects`/`outcome_effects` — with an in-panel Buy button; opened by clicking a token, closed via its `×`, Esc, or a click on empty canvas. Per-token buy button + right-click tooltips retired here (`TooltipManager` still used elsewhere). Resolves the long-standing constellation card text-overlap. Also null-guarded `DebugProgressionControl._sync` against a null `current_scene`. New `tests/integration/test_constellation_screen.gd`; 188 tests + headless green. *(Node icon art is placeholder-flat — regenerating the emblems + the 5 missing discipline icons via the pixel-engine skill is a deferred follow-up.)*
...
- [x] **Art pass** — placeholder colored rects replaced: CC0 combatant sprites, CC0 Gothicvania scene backgrounds (battle/campfire/menu), CC0 Kenney 9-slice UI skin, monogram pixel font. (Icon art for weapons/spells/nodes still partially pipeline-authored.)
