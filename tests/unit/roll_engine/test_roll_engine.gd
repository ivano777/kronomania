...
func test_keep_grade_1_keeps_1_die() -> void:
	var result: Dictionary = RollEngine.resolve(2, 6, 1)
	assert_eq(result["keep_count"] as int, 1, "grade 1 = keep 1 die")


func test_keep_grade_2_keeps_2_dice() -> void:
	var result: Dictionary = RollEngine.resolve(2, 6, 2)
	assert_eq(result["keep_count"] as int, 2, "grade 2 = keep 2 dice")


func test_keep_grade_3_keeps_3_dice() -> void:
	var result: Dictionary = RollEngine.resolve(3, 6, 3)
	assert_eq(result["keep_count"] as int, 3, "grade 3 = keep 3 dice")


func test_keep_capped_by_pool_size() -> void:
	# Pool has 1 die; even with grade 3 we can only keep 1.
	var result: Dictionary = RollEngine.resolve(1, 6, 3)
	assert_eq(result["keep_count"] as int, 1, "keep capped to pool size when pool < grade")


func test_negative_net_advantage_triggers_desperation() -> void:
	# tier=1, net_advantage=-1 → pool_size=0 → Desperation
	var result: Dictionary = RollEngine.resolve(1, 6, 1, 0, -1)
...
func test_all_dice_max_when_die_size_1() -> void:
	...
	var result: Dictionary = RollEngine.resolve(3, 1, 1)
...
func test_fervor_die_adds_to_total() -> void:
	...
	var result: Dictionary = RollEngine.resolve(1, 1, 1, 0, 0, 4)
...
func test_fervor_die_not_included_in_maxed_count() -> void:
	...
	var result: Dictionary = RollEngine.resolve(1, 1, 1, 0, 0, 1)
...
func test_fervor_maxed_flag() -> void:
	...
	var result: Dictionary = RollEngine.resolve(1, 6, 1, 0, 0, 1)
...
func test_post_keep_bonus_adds_to_total() -> void:
	...
	var result: Dictionary = RollEngine.resolve(1, 1, 1, 0, 0, 0, 0, 0, 4)
...
func test_seeded_calls_are_deterministic() -> void:
	seed(99991)
	var r1: Dictionary = RollEngine.resolve(3, 6, 2)
	seed(99991)
	var r2: Dictionary = RollEngine.resolve(3, 6, 2)
...
func test_flat_bonus_included_in_total() -> void:
	...
	var result: Dictionary = RollEngine.resolve(1, 1, 1, 7)
