...
| Mind Detonation placement roll uses pool=1, not tier | Deliberate weak scratch; gear-independent — `_get_cast_modifier` is captured but the placement call uses literal `1` with `net_advantage=0`; no `_pool_bonus` from gear |
...
| Explosion + echo use `_effective_tier(_player, null)` (Phase 1 interim) | Intentional placeholder; Phase 2 will freeze the chosen cast tool into the primed/echo status. Do not "fix" to null |
