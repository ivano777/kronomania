...
| Explosion uses frozen `cast_tier`/`cast_pool_bonus`/`cast_keep_bonus`/`cast_flat_bonus` from `mind_detonation_primed.stat_overrides` | Frozen at prime time; legacy statuses without keys fall back to `_effective_tier(_player, null)`. Do not read live `_player_cast_weapon` in `_detonate_mind_bomb` |
| Echo uses frozen `cast_tier`/`cast_pool_bonus`/`cast_flat_bonus` from `echoing_spell.stat_overrides`; keep NOT present | Focus keep baked into `current_kept_dice` at arming (decays with train); no separate `cast_keep_bonus` key in echo status |
