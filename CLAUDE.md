...
**UI bounds rule:** `tests/integration/test_ui_bounds.gd` asserts every visible Control of production scenes + debug widgets (panels expanded) fits the 640×360 viewport — new UI must keep it green. Overlay widgets: pin a corner, margin offsets only, `grow_horizontal/vertical` toward screen interior — never guessed width/height offsets (content min-size wins and spills offscreen). BattleScene mounts bottom-left debug widgets into a runtime `DebugDock` VBoxContainer — container layout, not hand-tuned offsets; add new battle debug widgets to the dock loop. MainMenuScene mounts `DebugDevMenu` (top-left, F12) = single "× DEV ×" button into `scenes/debug/DevHubScene.tscn` — full-screen maroon dev hub, visually distinct from player menu. DevHubScene owns the persistent SANDBOX: first jump seeds 999pt progression + `start_run()` (Campfire won't reset it) + `active_slot=0` (no autosave pollution), flag `DebugManager.sandbox_active`; hero picker switches `PlayerProgression.hero_sprite` anytime; jumps (Campfire hub / Constellation / Equipment / Training Dummy) all KEEP sandbox state so node/equip choices carry into the dummy fight; **closed environment**: while `sandbox_active`, every back/exit call site wraps its target in `DebugManager.nav_target(default)` → DevHub (forward nav untouched; MainMenu New-Game/Load clear the flag, so real play never redirects; release builds never set it — the debug-scene path stays dead code); Held-Anchor Editor returns via `DebugManager.return_scene` (empty = standalone CLI run, quits). DevHub EXPERIMENTS section (right column — left column is at the 360px height budget) hosts prototype toys: first is Aura Lab (`scenes/debug/experiments/AuraStudioScene.tscn`, in the bounds list) — studio around `AuraGen`/`AuraFX` (`scenes/battle/`, production-side so battle can adopt): aura spec derived from `hash(PRIVATE_KEY :: public_key)` seeding one RNG stream (palette schemes+profiles, baked particle textures, 1–6 motion layers, ripple rings with seeded geometry — circle/polygon/star/ellipse/dashed/rare jagged profile — plus 0–3 rare mutations; seed-rolled **rarity** (equipment vocabulary common→relic) sets the luck budget (mutation slots, eccentric chance, jagged weight), caller **tier** 1–4 (`generate_spec(key, tier)`) is a draw-free post-transform revealing a prefix of the rolled 3–6 layer potential + scaling amplitude — same key = same identity at every tier; **draw order in `generate_spec` is the contract** — reordering rng draws re-rolls every aura), unit-tested for determinism, tier identity/monotonicity + rare-path reachability. ConstellationScene's `DebugProgressionControl` is a left-column overlay (190px, mirrors NodeDetailPanel's right one): input picking ignores z_index and follows tree order, so it must be added LAST at scene root AND carry the highest z (60 > detail 50 > CardLayer 1) — drawn-on-top and click-target stay consistent; root `mouse_filter` IGNORE keeps the collapsed column click-transparent.
...
- **Pipeline art** (icons, spell/skill VFX, particles, effects, UI) = XPM frames in `tools/sprites/frames/`, compiled by `python tools/sprites/compile.py [--check] tools/sprites` (Pillow; scale 1). Install by convention (**no `.tres` edits**): icons → `assets/sprites/icons/{weapons|spells|nodes}/<key>.png` (key = name lowercased, spaces→underscores; node icons use `node_id`); held-equipment overlays → `assets/sprites/held/<key>.png` via `SpriteRegistry.get_held` (silent null on miss — art rolls out weapon-by-weapon; placement data-driven: per-item grip + optional `"planted"` (idle-only items, e.g. staff) + optional `"flip_h"`/`"flip_v"` art mirror (stacks on hero facing flip, rotation unaffected; a `<key>_back` variant carries its own flip independent of the front) + optional `"order"` draw-layers in `held/<key>.json` (`"hero;weapon;hand"` default, string = both hands or per-hand object; `hand` token = shared `_glove.png` fist riding the item's anchor+rotation (token position sets glove z — `hero;weapon;hand` = glove over weapon, `hero;hand;weapon` = glove behind weapon; omit token for cover items like shields); `weapon` before `hero` = renders behind body + swaps to `<key>_back.png` internal-side art (own grip if authored, else front's) if installed); shared glove art = `held/_glove.png` (+`_glove.json` grip), the heroine fist in a neutral gray ramp, no rarity tint; per-hero manifest `combatants/<hero>/held.json` — legacy `main`/`off`/`idle_bob` = idle, optional `"glove_off": {"main":[dx,dy,drot],"off":[dx,dy,drot]}` nudges the glove off the anchor + rotates it relative to the weapon angle (unflipped px/deg, mirrored at runtime; 2-element legacy = drot 0), `"anims"` = per-frame `[x, y, rot°]` anchor tables per animation (missing anim hides item; rotation pivots at grip, negated by runtime flip), optional per-hero override chain `"item_defaults"` (every item) / `"items": {"<key>": …}` (one item) carrying `flip_h`/`flip_v` (keyed by SHOWN art key — `_back` overridden independently), `pos`/`rot` deltas and `anims` tables (flip/pos/rot each scalar = both hands or per-hand `{"main":…,"off":…}`, missing hand falls through the chain) — per field, highest non-null tier wins outright (items > item_defaults > item's own json; replace never stack, misses fall through; an item `anims` miss falls back to hero tables, so it can't hide a hand) — resolution in `Combatant.held_override_field`+friends, authored via held-editor OVR panel, schema doc in SpriteRegistry — "main" = RIGHT/front hand, "off" = LEFT/back, coords + rotations in unflipped-sheet terms, held art authored VERTICAL (tip up, grip on symmetry axis, rot° = tilt from vertical); player-only; **swing smears**: frames whose anchor-rot delta ≥ threshold auto-replace held art with a procedural arc (`SmearGen` bakes from item art palette+silhouette, cached; optional `"smear"` params in `held/<key>.json`, hero-chain-overridable, `false` = off, never per-hand; glove keeps riding the anchor; toggle `DebugManager.smears_enabled` in battle AttackFX dock, live preview via held-editor "smear prev")); animated effects → `assets/sprites/effects/<Clip>_sheet.png`+`.json`, via `SpriteRegistry.get_effect_frames`. `.xpm`/`clips.json` writes auto-validated by PostToolUse hook; `out/` gitignored.
