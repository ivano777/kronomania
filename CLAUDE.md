...
| `mind_detonation_primed` / `hex_marked` dispatcher cases are no-ops | Breach-driven (`_check_mind_detonation` at Phase 2.1; hex amplification in `_resolve_attack`), not hook-driven; `pass` prevents silent misrouting |
| Mind Detonation placement roll uses pool=1, not tier | Deliberate weak scratch; the +1 pool bonus applies only to the explosion in `_detonate_mind_bomb` |
...
| hex amplification is `wounds_pending += 1` in `_resolve_attack` | Gated to `attacker_is_player and not defender_is_player and _has_status(defender, "hex_marked")` |
...
| `time_locked` is a two-phase status (armed→frozen) | Intentional — armed waits for the next player attack on this enemy; frozen locks that pool. The transition fires in `_resolve_attack` via the `did_breach` flag; `_end_of_round` reads `frozen_value` from `stat_overrides` |
| `_end_of_round` preserves frozen pools across `reset_guard()` | Intentional — Chrono-Tinkering freezes a guard; the block saves `frozen_value` from `stat_overrides`, resets, restores the pool, sets it rolled, decrements `skip_resets` |
| The armed→frozen transition fires for echoes and MD explosions too | Intentional — any player attack routed through `_resolve_attack` is "the next guard attacked"; echoes and explosions both pass `attacker_is_player=true` |
...
| Cantrip | Ingenuity die, Tier pool, no Fervor die, no escalation, usable during Burnout; via `node.spells` (Minor Studies: Arcane Bolt, Arcane Touch), `is_cantrip=true` |
| True spell | Ingenuity + optional aspect dice + real Fervor die; Spellcasting L1+ (Arcane Missile vs Stance, Arcane Mark vs Resolve); escalation = `primary_dice_maxed_count + (1 if fervor_maxed)` |
...
| Physical keep grade | `_physical_keep_grade()` = `maxi(_training_keep_grade(), physical_keep nodes)`. Physical Strike only. |
...
| Mind Detonation | True spell (L1 tier≥2, prereq Spellcasting L1). Placement = pool=1 Ingenuity scratch vs Stance (Fervor die, no bonuses); applies `mind_detonation_primed` (duration=3; freezes `fervor_at_prime`+`md_level`). At Phase 2.1, if primed + `_current_round_player_breaches["stance"]`, `_detonate_mind_bomb` removes the status and explodes vs Resolve via `_resolve_attack` using frozen Fervor (no escalation; bonuses via `_collect_spell_bonuses`). Fizzle logged on expiry. L2 (tier≥3, prereq ing_core L3): +1 explosion keep. Simplification: breach tracking is global (any Stance breach triggers all primed bombs). |
...
| Echoing Mind / Mind Lash | True spell (L1 tier≥3, prereq Spellcasting L1). Mind Lash `tags=["arcane","echo"]`. After cast, applies `echoing_spell` **on the player**. Each `end_of_round`, `_resolve_spell_echo` routes through `_resolve_attack(true,…)` with frozen `fervor_at_cast` and decremented `current_kept_dice` (starts cast_kept−1; −1 per echo; removed when next < 1). No escalation. `echo_flat = current_kept` at L2, 0 at L1. New cast overwrites the existing echo (latest wins — simplification). Echo is a full player attack: Hex + Mind Detonation interactions live. Cast with cast_kept=1 → no echo. |
