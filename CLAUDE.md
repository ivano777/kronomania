...
scenes/campfire/    # CampfireScene (rest, Equipment/Constellation nav, Give Up)
...
`InterruptHandler` = reactive hook registered per `CombatantState` at `start_combat()`. Two fire points, **separate dispatch paths**:
...
Lower priority fires first; two never contend (different fire points).
